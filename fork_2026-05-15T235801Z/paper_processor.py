...
# ══════════════════════════════════════════════════════════════════════════════
# HARDWARE CONTEXT
# RTX 5080  → 16 384 MiB VRAM   (GPU 0, CUDA0)
# RTX 3080  → 10 240 MiB VRAM   (GPU 1, CUDA1)
# Total     →  ~26 GB  (Ollama auto-spans or uses its own scheduler)
# RAM       → 128 GB   (no OOM concern for CPU offload)
# ══════════════════════════════════════════════════════════════════════════════

# Models in approximate VRAM-fit order (dual-GPU first, then single-GPU)
MODEL_TIERS = {
    # ── Dual-GPU tier  (~20–24 GB) ──────────────────────────────────────
    "xl_quality":   "qwen3.6:35b",                  # High reasoning, 23 GB
    "xl_reason":    "deepseek-r1:32b",              # Strong CoT, 19 GB
    "xl_code":      "qwen3-coder:30b",              # Best code generation, 18 GB
    # ── Mid tier  (~14–17 GB) ───────────────────────────────────────────
    "mid_code":     "qwen2.5-coder:14b-base-q6_K",  # 12 GB
    "mid_reason":   "deepseek-r1:14b-qwen-distill-q8_0", # 15 GB
    # ── Single-GPU tier  (≤12 GB) ───────────────────────────────────────
    "single":       "deepseek-r1:14b",              # 9 GB
    "single_code":  "qwen2.5-coder:14b",            # 9 GB
    # ── Fast fallback  (≤6 GB) ──────────────────────────────────────────
...
class Backend:
    ...
    def __init__(
        ...
        kv_cache_type: Optional[str] = "q8_0",
    ...
    def call(
        ...
        ctx = ctx_tokens or self.num_ctx
        if self.name == "ollama":
...
def map_reduce_chunks(
    ...
    for idx, chunk in enumerate(chunks, 1):
        ...
        summary = backend.call(prompt, model)
...
def main():
    ...
    ap.add_argument(
        "--ctx", type=int, default=8192, metavar="TOKENS",
        help="Default context length for LLM [default: 8192]",
    )
    ap.add_argument(
        "--no-flash", action="store_false", dest="flash", default=True,
        help="Disable Flash Attention (enabled by default)",
    )
    ap.add_argument(
        "--kv-cache", choices=["q8_0", "f16", "f32"], default="q8_0",
        help="KV cache quantization type [default: q8_0]",
    )
    ap.add_argument(
        "--single-runner", action="store_true",
        help="Enforce one model resident by setting keep_alive=0 (minimizes VRAM churn)",
    )
    ap.add_argument(
    ...
    backend       = Backend(
        ...
        default_model,
        num_ctx       = args.ctx,
        flash_attn    = args.flash,
        kv_cache_type = args.kv_cache,
        single_runner = args.single_runner
    )
    ...
    if args.backend == "ollama":
        print(f"  FlashAttn : {args.flash}")
        print(f"  KV Cache  : {args.kv_cache}")
        print(f"  SingleRun : {args.single_runner}")
    if args.reprocess:
