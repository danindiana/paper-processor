# Paper Processor: Dual-GPU Residency Fork 🦞

This is a high-performance fork of the `paper_processor` pipeline, architected specifically for dual-GPU systems (e.g., RTX 3080 + RTX 5080) to achieve maximum throughput through **strict GPU pinning** and **multi-model residency**.
...
- **Backend 1 (Port 11435):** Pinned to **GPU 1** (RTX 5080, 16GB).
...
| Component | GPU 0 (RTX 3080, 10GB) | GPU 1 (RTX 5080, 16GB) |
| :--- | :--- | :--- |
| **Reasoning** | `deepseek-r1:8b` (5.2GB) | `deepseek-r1:8b` (5.2GB) |
| **Code (C++)** | `qwen2.5-coder:7b` (4.4GB) | `ministral-3:8b` (6.0GB) |
| **Combined VRAM** | **9.6 GB** (tight — 8k ctx cap keeps actual usage ~9.5 GB) | **11.2 GB** (Fits!) |
...
- **Neon Diagrams:** Automated Graphviz generation with a dark/neon aesthetic. Robust rendering pipeline with stderr diagnostics and automatic sanitisation of common LLM DOT syntax errors.
